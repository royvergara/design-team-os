...
v0.8 — the release where the proof layer becomes shareable. Fifteen skills are live; the newest, `outcomes-scorecard`, renders the program-level scorecard into a self-contained page a Head of Design can view, print, and share — and still refuses to let activity read as a result. It builds on v0.7's scorecard template and the **owned bet** (v0.6), a recorded, named, review-dated way to proceed without evidence on purpose, so the exception is loud instead of silent. Every skill's primary refusal is encoded as a runnable fixture ([TESTING.md](TESTING.md)). Full version history in [CHANGELOG.md](CHANGELOG.md).
