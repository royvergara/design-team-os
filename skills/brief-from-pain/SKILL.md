...
The pre-registered, measurable success criteria, each tied to a signal the team can actually read (`validation-plan` designs the read when the signal isn't obvious), marked team-ratified not proposed. This is the section `brief-to-prompt`'s gate demands and `prototype-to-spec`'s Validation Record later scores against.
