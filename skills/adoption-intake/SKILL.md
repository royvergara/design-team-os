...
**Provenance, in the entry's own text.** Every back-filled line says so, in one fixed shape — `"<the signal, verbatim or counted> — <source>, <date of the source>; back-filled at intake <today> from <where you read it>"`, for example `"PostHog Activation: 41% of new workspaces never complete setup step 2, stable 90d — dashboard pull 2026-07-02; back-filled at intake 2026-09-03 from PRD, Evidence section"` — so a reader can always tell evidence reconstructed at adoption from evidence a gate produced. Beside the evidence lines, list the document's assertions you did not credit, one per line, quoted, under a `# recorded as unevidenced at intake:` comment in the same ledger, so the reader sees exactly what was read and what was set aside. This rides the schema's quote-inline rule; it needs no new field. And quote the signal, never the identity: counts and observations go in, named people do not.
...
One `design-os.work/<slug>.yaml` per live work item, and nothing else (see [templates/work-ledger.schema.md](../../templates/work-ledger.schema.md)). **Render each ledger in full, as YAML, in your reply** — the file is the deliverable; a narrative about what the file would say is not intake, it is a promise. **In the schema's shape, and only that shape:** the three blocks `intent`, `decision`, `value` with the fields the schema names. Never a `state:`, `status:`, `proven:` or `gates:` field — the schema has none, deliberately, because presence of the artifact is the state and a flag beside it is the checkmark this library refuses — and never a gate block you coined (`ratification:`, `prototype:`, `learning:`). Gaps go in a `# gaps:` comment block under the ledger, not in invented fields; a block with nothing genuine to carry is still written, holding only its gap comment, so the three-block shape is always visible. Each ledger carries:
...
Every ledger you claim to have written appears in full in the reply, as YAML in the schema's three-block shape with no status field of any kind, with every back-filled evidence line in the fixed shape above and the unevidenced assertions listed beside them; a reply that describes a ledger instead of showing it has not done intake. A reconstruction sheet's ticks are someone's checkmarks: credit the artifacts behind them, never the ticks. Every back-filled entry names its source and says it was back-filled. Every gap names what would close it. No status became a state, no assertion became evidence, no date moved. The well-run effort reads proven and the thin ones read open, each on its own artifacts. If the output could be read as a ranking of people, it failed, however accurate the numbers.
