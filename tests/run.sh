#!/usr/bin/env bash
# Design Team OS — skill smoke-test harness.
#
# Runs a FRESH Claude agent per fixture, feeds it a deliberately tempting input,
# and uses a second agent to judge whether the skill HELD ITS GATE under pressure.
# One command, a pass/fail line per fixture, non-zero exit if any fail.
#
# Usage:
#   tests/run.sh                 # every fixture
#   tests/run.sh prototype-to-spec   # one skill's fixtures only
#
# Requires the `claude` CLI on PATH. Configure the model with CLAUDE_TEST_MODEL
# (default: a cheap model — a gate that even a small model holds is a strong gate).
set -uo pipefail
...
# not inherit this repo's settings/skills. Prints the agent's response.
# The agent's stdout goes to a file (not the pipe) so command substitution
# captures only the final response; the timeout watcher is reaped so it never
# holds the pipe open for the full timeout.
run_agent() {
  local prompt="$1" tmp outfile pid watcher
  tmp="$(mktemp -d)"; outfile="$(mktemp)"
  # Feed the prompt on stdin, not as an argument: skill files start with `---`
...
The point of the test is that the input TEMPTS the skill to break a gate; a good
skill resists. Grade ONLY against the criteria below — not your own taste.
